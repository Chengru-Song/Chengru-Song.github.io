...
1. 确定性Policy：$\pi(s) = a$
2. Stochastic Policy: $\pi(s) = \mathbb{P}[A = a|S = s]$
...
在Model-free的场景下，习得一个episodic MDP模型的Value function的最大值 $V_{s_t}$ 和最优策略 $\pi_*(s)$ 。

2. 根本思想是什么

MC prediction: 根据state-Value function预测某个Policy在某个状态下的Value最大值。

MC control：the task that is **finding the optimal policy that maximize the value function** by alternating between policy evaluation and policy improvements. 
