...
- Training
    - 初始化：在语料库中，一共有V个不同的单词，初始化V个长度为m的向量，生成一个|V| \times m大小的矩阵；
    - 训练：每t个单词为一组，组成一个t \times m大小的矩阵作为Input Layer。经过hidden layers后经过最后的softmax得到一个概率vector，其中的第i个元素为下一个单词的概率。loss function是上文提到的L。
- Inference
